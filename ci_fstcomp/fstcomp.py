...
def fstcomp_df(df1: pd.DataFrame, df2: pd.DataFrame, exclude_meta=False, cmp_number_of_fields=True, columns=['nomvar','etiket','ni','nj','nk','dateo','ip1','ip2','ip3','deet','npas','grtyp','ig1','ig2','ig3','ig4'], print_unmatched=False,e_max=0.0001,e_c_cor=0.00001) -> bool:
    ...
    if cmp_number_of_fields:
        if len(df1.index) != len(df2.index):
            logging.error('file 1 (%s) and file 2 (%s) dont have the same number of records' % (len(df1.index),len(df2.index)))
    ...
    if len(common.index) != 0:
        ...
        if not df1.empty:
            logging.error('A \n%s'%df1[['nomvar', 'etiket','ni', 'nj', 'nk', 'dateo', 'ip1', 'ip2', 'ip3', 'deet', 'npas', 'grtyp', 'ig1', 'ig2', 'ig3', 'ig4']].reset_index(drop=True).to_string())
        logging.info('----------')
        if not df2.empty:
            logging.error('B \n%s'%df2[['nomvar', 'etiket', 'ni', 'nj', 'nk', 'dateo', 'ip1', 'ip2', 'ip3', 'deet', 'npas', 'grtyp', 'ig1', 'ig2', 'ig3', 'ig4']].reset_index(drop=True).to_string())
    ...
    if len(missing.index):
        ...
        logging.error('\n%s'%missing[['nomvar', 'etiket', 'ip1', 'ip2', 'ip3']].to_string(header=False, formatters={'level': '{:,.6f}'.format}))
        return False
